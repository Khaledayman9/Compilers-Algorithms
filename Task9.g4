...
t [int r, int l] returns [int check, int m]: '#' n[$r, 1, $l] {$check = $n.check; $m = $n.m;}
| {$check = 1; $m = 1;};

n [int r, int c, int l] returns [int check, int m]: d[$r, 1] t[$r+1, $l] {$check = $d.check * $t.check; $m = equals($d.l, $l) * $t.m;};

d [int r, int c] returns [int l, int check]: '0' d1=d[$r, $c+1] {$l = $d1.l; $check = 1 - equals($c, $r) * $d1.check;}
